...
import { wsUrl } from '../WorkspaceContext.jsx'
...
export default function WorkspaceSettingsModal({ workspaceName, profile, wsEntry, onClose }) {
  ...
  const wsType = profile?.type || (typeof wsEntry === 'object' ? wsEntry?.type : null) || 'local'
  const wsPath = typeof wsEntry === 'string' ? wsEntry : wsEntry?.path || ''
  const isHuman = wsType === 'human'

  // Load per-workspace settings
  useEffect(() => {
    fetch(wsUrl('/api/settings', workspaceName))
      .then(r => r.json())
  ...
  function handleIndexNow() {
    setIndexingNow(true)
    fetch(wsUrl('/api/settings/index-now', workspaceName), { method: 'POST' })
      .catch(console.error)
      .finally(() => {
  ...
  return (
    ...
    >
      <div className="bg-base-200 border border-base-300 rounded-xl shadow-2xl w-full max-w-[420px] max-h-[80vh] flex flex-col mx-4">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-base-300 shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-base-content">{workspaceName}</span>
            {isHuman && (
              <span className="text-[10px] px-1.5 py-0 rounded bg-rose-500/15 text-rose-400 font-medium">
                human
              </span>
            )}
        ...
        {/* Path */}
        {wsPath && (
          <p className="text-[11px] text-neutral-content opacity-35 px-5 pt-2 truncate" title={wsPath}>
            {wsPath}
          </p>
        )}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          {isHuman ? (
            <p className="text-sm text-neutral-content opacity-60">
              Agent settings don't apply to human workspaces.
            </p>
          ) : !settings ? (
              ...
              <section>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {(bi?.excluded_dirs || []).map(dir => (
                      <span
                        key={dir}
                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-base-100 text-xs text-base-content"
                      >
                        {dir}
                        <button
                ...
                <label className="flex items-center justify-between gap-3 cursor-pointer mb-1">
                  ...
                  <input
                    ...
                    checked={bi?.enabled ?? false}
                    onChange={e => saveSettings({
                      ...settings,
                      background_index: { ...bi, enabled: e.target.checked },
                    })}
                  />
                </label>
                <p className="text-xs text-neutral-content opacity-50 mb-3">
                  Indexes automatically at idle priority.
                </p>
                ...
                {bi?.enabled && (
                  <label className="flex items-center justify-between gap-3 mb-3">
                    ...
                    <select
                      ...
                      onChange={e => saveSettings({
                        ...settings,
                        background_index: { ...bi, interval_minutes: Number(e.target.value) },
                      })}
                    >
                ...
                <label className="flex items-center justify-between gap-3 mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      max={300}
                      step={10}
                      value={mcp?.query_timeout_seconds ?? 60}
                      onChange={e => {
